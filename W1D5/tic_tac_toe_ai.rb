...
class TreeNode
  ...
  def max_action(target, depth)
    ...
    @children.each do |child|
      child_result = child.min_action(target)
      if child_result > min_value
        min_value = child_result
        best_child = child
      end
    end
    return best_child if depth == 1
    min_value
  end

  def min_action(target)
    return minimax_result(target) if @children.empty?
    max_value = 3
    @children.each do |child|
      child_result = child.max_action(target, 0)
...
class TicTacToe
  ...
  def build_game_tree
    ...
    until frontier.empty?
     ...
     current_node.children.each { |child| frontier << child }
...
class ComputerPlayer
  ...
  def winner_move(game, mark)
    current_node = game.game_tree.bfs(game.board)
    p current_node
    best_child = current_node.minimax(mark)
    p best_child.pos
    return position = best_child.pos
    # no winning move
    nil
  end
